...
void Character::Update()
{
    ...
    if (!m_EffectHandles.empty())
    {
        ...
        for (auto it = m_EffectHandles.begin(); it != m_EffectHandles.end(); )
        {
            ...
            // エフェクトの再生を1フレーム分進める (UpdateHandleは1 = 1/60秒).
            ...
            {
                std::string msg = std::string("エフェクトハンドル ") + std::to_string(handle) + std::string(" が期限切れで削除 (経過=") + std::to_string(entry.AgeFrames) + std::string(")");
